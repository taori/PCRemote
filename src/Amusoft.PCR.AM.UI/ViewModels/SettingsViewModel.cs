...
public partial class SettingsViewModel : PageViewModel, INavigationCallbacks
{
    ...
    {
	    if (await _userInterfaceService.GetPromptTextAsync(Translations.Settings_NewPort, Translations.Settings_AddNumber, maxLength: 5, keyboard: Keyboard.Numeric) is var input && input is null)
		    return;
	    ...
	    if (int.TryParse(input, out var number))
	    {
		    ...
		    {
			    await _userInterfaceService.DisplayAlertAsync(Translations.Generic_Error, Translations.Settings_PortAlreadyExists);
		    });
		}
	    else
	    {
		    await _userInterfaceService.DisplayAlertAsync(Translations.Generic_Error, Translations.Settings_FailedToInsertPort);
	    }
    }
}